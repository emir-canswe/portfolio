  ...
  return (
    <section id="experience" className="py-20 px-4 sm:px-6 lg:px-8 relative z-10">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="relative pl-6 sm:pl-8 border-l-2 border-cyan-500/30 space-y-12">
          {experiences.map((exp, idx) => (
            <div key={idx} className="relative group">
              {/* Glowing Timeline Node */}
              <div className="absolute -left-[31px] sm:-left-[39px] top-1.5 w-6 h-6 rounded-full bg-[#0B0F1A] border-2 border-cyan-400 flex items-center justify-center group-hover:shadow-[0_0_15px_rgba(0,242,255,0.7)] group-hover:scale-110 transition-all">
                <span className="w-2 h-2 rounded-full bg-cyan-400 group-hover:animate-ping" />
              </div>

              {/* Experience Card */}
              <div className="hud-glass hud-glass-hover rounded-xl p-5 sm:p-7 hud-corner-brackets">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-4 border-b border-slate-800">
                  <div>
                    <span className="inline-block px-2.5 py-0.5 rounded text-[11px] font-mono font-semibold bg-cyan-500/10 text-cyan-300 border border-cyan-500/30 mb-2">
                      {exp.type[lang]}
                    </span>
                ...
                <div className="mt-5 pt-4 border-t border-slate-800/80 flex flex-wrap gap-1.5">
                  {exp.tech.map((t, tIdx) => (
                    <span
                      key={tIdx}
                      className="px-2 py-0.5 rounded bg-slate-900/90 text-slate-300 border border-slate-700/60 text-[11px] font-mono hover:border-cyan-500/50 hover:text-cyan-300 transition-colors"
                    >
                      {t}
                    </span>
                  ))}
